...
/**
 * Get cached my-likes data from localStorage
 * Validates that cache belongs to current user to prevent data leakage
 */
const getCachedMyLikes = () => {
  try {
    const cached = localStorage.getItem(MY_LIKES_CACHE_KEY);
    if (cached) {
      ...
      if (userId && currentUserId && String(userId) !== String(currentUserId)) {
        localStorage.removeItem(MY_LIKES_CACHE_KEY);
        return null;
      }
...
/**
 * Save my-likes data to localStorage cache with user ID for validation
 */
const setCachedMyLikes = (data) => {
  try {
    const userId = getCurrentUserId();
    localStorage.setItem(MY_LIKES_CACHE_KEY, JSON.stringify({
      data,
      timestamp: Date.now(),
...
const MyLikePage = () => {
  ...
  const fetchMyLikesData = useCallback(async (forceRefresh = false) => {
    if (!userId) {
      setLoading(false);
      return;
    }
    ...
    setError(null);
    try {
      const { data } = await api.get(`/likes/me/my-likes`, {
      ...
      setCachedMyLikes(list);
    } catch (err) {
      setError(err.response?.data?.detail || err.message);
      console.error('Error fetching my likes data:', err);
      // Only show error if we don't have cached data
      if (!getCachedMyLikes()?.data) {
        setError(err.response?.data?.detail || err.message);
      }
    } finally {
      setLoading(false);
          ...
          <div>
            {profiles.length === 0 ? (
              <h2 style={styles.sectionTitle}>
                Time to Spark an Aura!<br />Your 'liked' list is empty! Head to Discover and start finding profiles that resonate with your AURA.
              </h2>
            ) : (
              <>
